...
namespace Example.Events.ServiceBus
{
    public class ServiceBusTopicEventSender : IEventSender, IAsyncDisposable
    {
        ...
        public Task<Result<Unit>> SendAsync<T>(string topicName, T @event) => Task.Run(async () =>
        {
            ...
            string jsonAsString = Encoding.UTF8.GetString(encodedCloudEvent.Span);
            Console.WriteLine($"Encoded CloudEvent JSON: {jsonAsString}");
            ...
            Result<Unit> result = new Result<Unit>(Unit.Default);
            return result;
        });
