...
{
    internal class ModifyShoppingCartWorkflow
    {
        private readonly IOrdersRepository ordersRepository;
        private readonly IProductsRepository productsRepository;
        private readonly ILogger<ModifyShoppingCartWorkflow> logger;

        public ModifyShoppingCartWorkflow(IOrdersRepository ordersRepository, IProductsRepository productsRepository, ILogger<ModifyShoppingCartWorkflow> logger)
        {
        ...
        public async Task<IShoppingCartModifiedEvent> ExecuteAsync(ProcessOrderCommand command)
        {
            PaidShoppingCart paidCart = 
            UnvalidatedShoppingCart unvalidatedCart = new UnvalidatedShoppingCart(command.InputShoppingCart); //din paidCart
            ...
            return await result.Match(
                    Left: products => GenerateFailedEvent(products) as IShoppingCartModifiedEvent,
                    Right: paidCart => new ShoppingCartSucceededEvent(paidCart.Csv, paidCart.PayDate)
                );
        }
        ...
        private ShoppingCartFailedEvent GenerateFailedEvent(IShoppingCart cart) =>
            cart.Match<ShoppingCartFailedEvent>(
